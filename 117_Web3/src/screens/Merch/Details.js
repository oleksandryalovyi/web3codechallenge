                    ...
                    <div className="flex mb-4">
                      {
                        merchDetails.colors.map((item, index) => 
                          <div 
                            ...
                            >
                            <img className="w-full" src={BASEURL + "/images/1.png"} alt={item.color} />
                          </div>
                        )
                      }
                    </div>
                    <div className="flex flex-wrap mb-4">
                      ...
                      }
                      {
                        tokenList.map((item, index) => {
                          console.log(selectedColor.sizes[sizeIndex], index);
                          const price = 0; 
                          if (selectedColor.sizes[sizeIndex].merchPrice.length > index + 1)
                            price = selectedColor.sizes[sizeIndex].merchPrice[index + 1].price;
                          if (price !== 0) {
                            return (
                              <div 
                                key={index} 
                      ...
                      >
                        {
                          selectedColor.sizes.map((item, index) =>
                            <option key={index} className="color-black" value={index} disabled={item.amount === 0}>
                              Size: {item.size},&nbsp;Price: {item.merchPrice[selectedToken].price}&nbsp;{item.merchPrice[selectedToken].currency}
                            </option>
                          )
                        }
                      </Select>
